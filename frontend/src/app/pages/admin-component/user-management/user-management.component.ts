...
export class UserManagementComponent implements OnInit {
  ...
  verifyUser(userId: string, userName: string) {
    this.userService.verifyUserAccount(userId).subscribe({
      next: (res) => {
        alertify.success(`${userName} verified successfully`);
        this.loadAllData();
      },
      error: (err) => {
        alertify.error(err.error?.message || 'Failed to verify user');
      }
    });
  }
}
